...
class Calculator:
    ...
    def equal_button_press(self):
        if self.add_trigger or self.sub_trigger or self.mult_trigger or self.div_trigger:
            if self.add_trigger:
                res = self.calc_value + float(self.entry_value.get())
            elif self.sub_trigger:
                res = self.sub_value - float(self.entry_value.get())
            elif self.mult_trigger:
                res = self.calc_value * float(self.entry_value.get())
            elif self.div_trigger:
                res = self.sub_value / float(self.entry_value.get())

        print(self.calc_value, ' ', float(self.entry_value.get()), ' ', res)
        self.number_entry.delete(0, 'end')
        self.number_entry.insert(0, res)
    ...
    def __init__(self, root):
        ...
        self.button_mult = ttk.Button(root, text="x", command=lambda: self.math_button_press("*")).grid(row=2, column=3)

        # ---- 3rd Row ----

        self.button1 = ttk.Button(root, text="1", command=lambda: self.button_press("4")).grid(row=3, column=0)
        self.button2 = ttk.Button(root, text="2", command=lambda: self.button_press("5")).grid(row=3, column=1)
        self.button3 = ttk.Button(root, text="3", command=lambda: self.button_press("6")).grid(row=3, column=2)
        self.button_add = ttk.Button(root, text="+", command=lambda: self.math_button_press("+")).grid(row=3, column=3)

        # ---- 4th Row ----

        self.buttonAC = ttk.Button(root, text="AC", command=lambda: self.button_press("AC")).grid(row=4, column=0)
